...
fn main() {
    ...
    let binary_name = match target_os.as_str() {
        "windows" => format!("rapitas-backend-{}-pc-windows-{}.exe", target_arch, target_env),
    ...
    let alternative_names: Vec<&str> = match target_os.as_str() {
        "windows" => vec!["rapitas-backend.exe", "rapitas-backend-x86_64-pc-windows-msvc.exe"],
    ...
    if source_path.exists() {
        println!("cargo:rustc-env=RAPITAS_BACKEND_PATH={}", source_path.display());
        found = true;
    } else {
        ...
        for alt_name in alternative_names {
            ...
            if alt_path.exists() {
                println!("cargo:rustc-env=RAPITAS_BACKEND_PATH={}", alt_path.display());
                found = true;
                break;
            }
        }
    ...
    if !found {
        ...
        println!("cargo:warning=Backend binary not found at: {}", source_path.display());
        println!("cargo:warning=Looked for alternatives in: binaries/");
        if let Ok(entries) = fs::read_dir("binaries") {
            for entry in entries.flatten() {
                println!("cargo:warning=  Found: {}", entry.file_name().to_string_lossy());
            }
        }
    }
