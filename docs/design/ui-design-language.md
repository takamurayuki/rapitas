...
- **Status hues (one meaning = one hue, app-wide)**: running / in-progress / active phase = **blue** (`blue-600 dark:blue-400`, bg `blue-50 dark:blue-900/30`); waiting-on-user & warning = **amber**; error / blocked = **red**; success / completed = **green**. Blue is reserved for the *running state* and is distinct from the indigo *interactive accent* — do not swap them. Sources of truth: `ui/status-card/StatusCard.tsx`, `workflow/WorkflowStatusIndicator.tsx`, `feature/tasks/config/StatusConfig.tsx`, `ui/alert.tsx`.
