...
async function checkStagedFiles() {
  ...
  if (frontendFiles.length > 0) {
    ...
    if (prettierCheck.error) {
      ...
      const lines = prettierCheck.stdout.split('\n');
      lines.forEach((line) => {
        if (line.trim()) {
          log(`  │     ${line}`, 'red');
        }
      ...
      log(
        '  │  💡 修正方法: cd rapitas-frontend && npx prettier --write <ファイル名>',
        'yellow'
      );
      hasErrors = true;
    } else {
  ...
  if (backendFiles.length > 0) {
    ...
    if (prettierCheck.error) {
      ...
      log(
        '     💡 修正方法: cd rapitas-backend && npx prettier --write <ファイル名>',
        'yellow'
      );
      hasErrors = true;
    } else {
  ...
  if (hasErrors) {
    ...
    if (backendFiles.length > 0) {
      log(
        '   cd rapitas-backend && npx prettier --write "**/*.ts"',
        'cyan'
      );
    }
