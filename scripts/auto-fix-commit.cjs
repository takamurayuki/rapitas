...
function stageFiles(files) {
  ...
  if (failCount > 0) {
    log(`  ⚠️  ${failCount}個のファイルのステージングに問題がありました（無視して続行）`, 'yellow');
  }
...
async function main() {
  ...
  if (frontendFiles.length > 0) {
    ...
    const useGlob = frontendFiles.length > 50;
    ...
    // Prettier
    log('  ├─ Prettier実行...', 'cyan');
    const prettierResult = exec(`cd rapitas-frontend && npx prettier --write ${prettierTarget}`);
    ...
    // ESLint (with --max-warnings to allow warnings)
    log('  └─ ESLint --fix 実行...', 'cyan');
    const eslintResult = exec(
      `cd rapitas-frontend && npx eslint --fix --max-warnings=9999 --config eslint.config.mjs ${eslintTarget}`,
    );
  ...
  if (backendFiles.length > 0) {
    ...
    // ファイル数が多い場合はglobパターンを使用
    const useGlob = backendFiles.length > 50;
    const prettierTarget = useGlob
      ? '"**/*.ts"'
      : `"${backendFiles.map((f) => f.replace('rapitas-backend/', '')).join('" "')}"`;

    const prettierResult = exec(`cd rapitas-backend && npx prettier --write ${prettierTarget}`);
