...
export const approveRoutes = new Elysia()
  ...
  .post('/:id/approve', async (context) => {
      ...
      orchestrator
        ...
        .catch(async (error) => {
          if (isShutdownError(error)) {
            log.warn(
              `[approve] Server is shutting down — skipping error notification for task ${task.id}`,
            );
            return;
          }
    ...
    if (approval.requestType === 'code_review') {
      ...
      // Target the theme's configured default branch (consistent with the
      // workflow-auto-commit path) instead of letting createPullRequest fall back
      // to the global develop>main>master auto-detect, which ignores the theme.
      const themeForBase = task.themeId
        ? await prisma.theme.findUnique({
            where: { id: task.themeId },
