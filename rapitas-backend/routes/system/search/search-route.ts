  ...
  try {
    ...
    if (types.includes('task')) {
      try {
        ...
        // NOTE: P2021 means the Task table does not yet exist in the SQLite DB.
        // The desktop-sqlite self-heal (config/desktop-sqlite.ts) repairs this on
        // the next server restart. Skip task results rather than failing the
        // entire search with HTTP 500.
        if ((err as PrismaClientKnownRequestError).code === 'P2021') {
          log.warn({ err }, 'Task table missing from SQLite DB — skipping task search results');
        } else {
          throw err;
        }
      }
    ...
    if (types.includes('note')) {
      try {
        ...
        // NOTE: P2021 means the PomodoroSession or TimeEntry table does not yet exist in the SQLite DB.
        // The desktop-sqlite self-heal (config/desktop-sqlite.ts) repairs this on
        // the next server restart. Skip note results rather than failing the
        // entire search with HTTP 500.
        if ((err as PrismaClientKnownRequestError).code === 'P2021') {
          log.warn({ err }, 'Note table missing from SQLite DB — skipping note search results');
        } else {
          throw err;
        }
      }
    ...
    if (types.includes('resource')) {
      try {
        ...
        // NOTE: P2021 means the Resource table does not yet exist in the SQLite DB.
        // The desktop-sqlite self-heal (config/desktop-sqlite.ts) repairs this on
        // the next server restart. Skip resource results rather than failing the
        // entire search with HTTP 500.
        if ((err as PrismaClientKnownRequestError).code === 'P2021') {
          log.warn(
            { err },
            'Resource table missing from SQLite DB — skipping resource search results',
          );
        } else {
          throw err;
        }
