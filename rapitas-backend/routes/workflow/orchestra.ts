...
export const orchestraRoutes = new Elysia()
   ...
   */
  .post(
    '/workflow/orchestra/run-subtasks',
    async ({ body }) => {
          ...
          await prisma.workflowQueueItem
            .updateMany({
              ...
              data: {
                ...
                errorMessage: 'Re-run by user',
              },
            })
            .catch((err) => {
              log.warn(
                { err, taskId },
                '[Orchestra] Failed to cancel subtask queue items on re-run',
              );
            });
        }
      }
    ...
    {
      body: t.Object({ taskId: t.Number() }, { additionalProperties: false }),
    },
  );
