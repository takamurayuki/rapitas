/**
 * Tests for verify-self-repair-feedback's pure feedback-block helpers.
 *
 * These cover the trio behind writeRepairFeedback: reason sanitisation (no
 * numeric failure tallies survive), marker wrapping, and replace-not-stack
 * merging — which stops a past rejection from poisoning the next
 * validateVerify cycle (task 494's repair loop). No prisma/logger mocking is
 * needed here — these functions have no DB or workflow-file dependency.
 */
...
import {
  ...
  REPAIR_FEEDBACK_END,
} from './verify-self-repair-feedback';
