...
/**
 * Attempt to repair and parse a potentially truncated JSON string.
 */
function parseJSONSafe<T>(jsonStr: string): T {
  ...
  try {
    ...
    // On parse failure, attempt repair
  }
  ...
  // Remove trailing commas again (after closing strings)
  fixed = fixed.replace(/,\s*$/, '');
...
/**
 * Analyze a task and propose subtasks via AI.
 */
...
/**
 * Generate execution instructions for subtasks.
 */
