...
describe("GET /tasks/:id/prompts", () => {
  ...
  test("タスクのプロンプト一覧を返すこと", async () => {
    ...
    const prompts = [
      { id: 1, taskId: 1, optimizedPrompt: "prompt1" },
    ];
  ...
  test("タスクが見つからない場合エラーを返すこと", async () => {
    ...
    const res = await app.handle(
      new Request("http://localhost/tasks/999/prompts"),
    );
    const body = await res.json();
...
describe("DELETE /prompts/:id", () => {
  ...
  test("プロンプトを削除すること", async () => {
    ...
    const res = await app.handle(
      new Request("http://localhost/prompts/1", { method: "DELETE" }),
    );
    const body = await res.json();
  ...
  test("存在しないIDで404を返すこと", async () => {
    ...
    const res = await app.handle(
      new Request("http://localhost/prompts/999", { method: "DELETE" }),
    );
    const body = await res.json();
