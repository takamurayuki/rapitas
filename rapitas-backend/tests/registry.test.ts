...
describe('AgentRegistry', () => {
  ...
  describe('能力ベースフィルタ', () => {
    test('getProvidersByCapabilityで特定能力のプロバイダーを取得すること', () => {
      registry.registerProvider(createMockProvider('p1', { codeGeneration: true }));
      registry.registerProvider(createMockProvider('p2', { codeGeneration: false }));
      registry.registerProvider(createMockProvider('p3', { codeGeneration: true }));
  ...
  describe('エージェント管理', () => {
    ...
    test('存在しないプロバイダーでエラーを投げること', () => {
      expect(() => registry.createAgent({ providerId: 'unknown' } as AgentProviderConfig)).toThrow(
  ...
  describe('最適プロバイダー選択', () => {
    test('必須能力を持つプロバイダーを返すこと', async () => {
      ...
      const best = await registry.selectBestProvider(['codeGeneration', 'fileWrite']);
      expect(best).not.toBeNull();
      expect(best!.providerId).toBe('p1');
    });

    test('候補がない場合nullを返すこと', async () => {
      registry.registerProvider(createMockProvider('p1', { codeGeneration: false }));
  ...
  describe('ヘルスチェック', () => {
    ...
    test('ヘルスチェック失敗時にエラー情報を返すこと', async () => {
      ...
      provider.healthCheck = mock(() => Promise.reject(new Error('Connection refused')));
