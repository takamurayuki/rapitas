...
describe("NginxLogParser", () => {
  ...
  test("user=-の場合undefinedになること", () => {
    const log =
      '10.0.0.1 - - [01/Jan/2026:10:30:00 +0000] "GET / HTTP/1.1" 200 100 "-" "curl"';
  ...
  test("ステータスコード500以上でERRORレベルになること", () => {
    const log =
      '10.0.0.1 - - [01/Jan/2026:10:30:00 +0000] "GET / HTTP/1.1" 500 100 "-" "curl"';
    const result = parser.parse(log);
  ...
  test("ステータスコード400台でWARNレベルになること", () => {
    const log =
      '10.0.0.1 - - [01/Jan/2026:10:30:00 +0000] "GET / HTTP/1.1" 404 100 "-" "curl"';
    const result = parser.parse(log);
...
describe("ApacheCombinedLogParser", () => {
  ...
  test("size=-の場合0になること", () => {
    const log =
      '10.0.0.1 - - [01/Jan/2026:10:30:00 +0000] "HEAD / HTTP/1.1" 304 - "-" "curl"';
    const result = parser.parse(log);
...
describe("WindowsEventLogParser", () => {
  ...
  test("CSVフォーマットをパースできること", () => {
    const log =
      '"Error","2026-01-01 10:00:00","Application","1234","None","An error occurred"';
  ...
  test("mapEventLevelが正しくマッピングすること", () => {
    const info =
      '"Information","2026-01-01 10:00:00","App","1","None","Info msg"';
    const warn =
      '"Warning","2026-01-01 10:00:00","App","1","None","Warn msg"';
    const critical =
      '"Critical","2026-01-01 10:00:00","App","1","None","Critical msg"';
...
describe("PostgreSQLLogParser", () => {
  ...
  test("PostgreSQLログをパースできること", () => {
    const log =
      "2026-01-01 10:00:00.123 UTC [12345] LOG:  database system is ready";
    const result = parser.parse(log);
    expect(result).not.toBeNull();
  ...
  test("DEBUGレベルをマッピングすること", () => {
    const log =
      "2026-01-01 10:00:00.123 UTC [100] DEBUG:  query plan generated";
    const result = parser.parse(log);
  ...
  test("ERRORレベルをマッピングすること", () => {
    const log =
      "2026-01-01 10:00:00.123 UTC [100] ERROR:  relation does not exist";
    const result = parser.parse(log);
    expect(result!.level).toBe(LogLevel.ERROR);
  });
  ...
  test("FATALレベルをマッピングすること", () => {
    const log =
      "2026-01-01 10:00:00.123 UTC [100] FATAL:  could not open file";
    const result = parser.parse(log);
  ...
  test("WARNINGレベルをマッピングすること", () => {
    const log =
      "2026-01-01 10:00:00.123 UTC [100] WARNING:  table has no primary key";
    const result = parser.parse(log);
    expect(result!.level).toBe(LogLevel.WARN);
  });
});
...
describe("PythonLogParser", () => {
  ...
  test("Python loggingフォーマットをパースできること", () => {
    const log =
      "2026-01-01 10:00:00,123 - myapp.module - INFO - Request processed";
    expect(parser.canParse(log)).toBe(true);
    const result = parser.parse(log);
    expect(result).not.toBeNull();
  ...
  test("CRITICALレベルをFATALにマッピングすること", () => {
    const log =
      "2026-01-01 10:00:00,000 - root - CRITICAL - System shutdown";
    const result = parser.parse(log);
    expect(result!.level).toBe(LogLevel.FATAL);
  });
  ...
  test("WARNINGレベルをWARNにマッピングすること", () => {
    const log =
      "2026-01-01 10:00:00,000 - app - WARNING - Deprecated function";
    const result = parser.parse(log);
    expect(result!.level).toBe(LogLevel.WARN);
  });
});
