...
describe('PrismaDataLoader', () => {
  ...
  test('存在しないIDでnullを返すこと', async () => {
    const loader = new PrismaDataLoader<{ name: string }>(async () => new Map(), 100, 5);
...
describe('Advanced PrismaOptimizer Tests', () => {
  describe('複雑なバッチ処理', () => {
    test('非同期エラーを適切に処理すること', async () => {
      ...
      await PrismaOptimizer.batchOperation([1, 2, 3, 4, 5], 2, async (batch) => {
        if (batch.includes(3)) {
          errorCount++;
...
describe('QueryOptimizers 拡張テスト', () => {
  ...
  test('searchTasksで複雑な検索条件を構築できること', () => {
    const complexSearch = QueryOptimizers.searchTasks('urgent task', {
      status: 'in_progress',
      priority: 'high',
      assignedUserId: 123,
      projectId: 456,
