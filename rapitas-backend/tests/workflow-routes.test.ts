...
mock.module("../services/agents/agent-orchestrator", () => ({
  AgentOrchestrator: {
    getInstance: () => ({
      ...
      createCommit: mock(() => Promise.resolve({ hash: "abc123", branch: "main", filesChanged: 1 })),
...
describe("GET /workflow/tasks/:taskId/files", () => {
  ...
  test("ワークフローファイル一覧を返すこと", async () => {
    ...
    const res = await app.handle(
      new Request("http://localhost/workflow/tasks/1/files"),
    );
  ...
  test("無効なタスクIDで400を返すこと", async () => {
    const res = await app.handle(
      new Request("http://localhost/workflow/tasks/abc/files"),
    );
    const body = await res.json();
  ...
  test("存在しないタスクで404を返すこと", async () => {
    ...
    const res = await app.handle(
      new Request("http://localhost/workflow/tasks/999/files"),
    );
    const body = await res.json();
