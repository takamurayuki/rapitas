...
describe("GET /screenshots/:filename", () => {
  ...
  test("パストラバーサルを含むファイル名で400を返すこと (..)", async () => {
    const res = await app.handle(
      new Request("http://localhost/screenshots/..%2Fetc%2Fpasswd")
    );
  ...
  test("スラッシュを含むファイル名で400を返すこと", async () => {
    const res = await app.handle(
      new Request("http://localhost/screenshots/path%2Ffile.png")
    );
  ...
  test("バックスラッシュを含むファイル名で400を返すこと", async () => {
    const res = await app.handle(
      new Request("http://localhost/screenshots/path%5Cfile.png")
    );
