...
const mockPrisma = {
  favoriteDirectory: {
    ...
    create: mock(() => Promise.resolve({ id: 1, path: '/test-dir', name: 'test-dir' })),
...
describe('GET /directories/favorites', () => {
  ...
  test('お気に入り一覧を返すこと', async () => {
    ...
    const res = await app.handle(new Request('http://localhost/directories/favorites'));
...
describe('DELETE /directories/favorites/:id', () => {
  ...
  test('削除時にエラーが発生した場合にエラーを返すこと', async () => {
    mockPrisma.favoriteDirectory.delete.mockRejectedValue(new Error('Record not found'));
