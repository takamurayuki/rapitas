...
describe('handleApiRecoveryHealthCheck', () => {
  it('returns a healthy payload when the DB query succeeds', async () => {
  ...
  it('returns a 503 unhealthy response when the DB query fails', async () => {
    mockQueryRaw.mockImplementationOnce(() => Promise.reject(new Error('connection refused')));
    ...
    expect(body.mode).toBe('api-recovery');
    expect(body.error).toBe('connection refused');
  });
});
