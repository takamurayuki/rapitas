...
const { templatesRoutes } = await import('../../../routes/organization/templates');
...
describe('GET /templates/:id', () => {
  ...
  test('IDでテンプレートを取得すること', async () => {
    ...
    const res = await app.handle(new Request('http://localhost/templates/1'));
    const body = await res.json();
  ...
  test('存在しないIDでnullを返すこと', async () => {
    ...
    const res = await app.handle(new Request('http://localhost/templates/999'));
...
describe('DELETE /templates/:id', () => {
  ...
  test('テンプレートを削除すること', async () => {
    ...
    const res = await app.handle(new Request('http://localhost/templates/1', { method: 'DELETE' }));
