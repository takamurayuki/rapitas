...
const { milestonesRoutes } = await import('../../../routes/organization/milestones');
...
describe('GET /milestones', () => {
  ...
  test('全マイルストーンを返すこと', async () => {
    ...
    const res = await app.handle(new Request('http://localhost/milestones'));
  ...
  test('projectIdでフィルタリングできること', async () => {
    ...
    const res = await app.handle(new Request('http://localhost/milestones?projectId=1'));
...
describe('GET /milestones/:id', () => {
  ...
  test('IDでマイルストーンを取得すること', async () => {
    ...
    const res = await app.handle(new Request('http://localhost/milestones/1'));
    const body = await res.json();
  ...
  test('無効なIDで400を返すこと', async () => {
    const res = await app.handle(new Request('http://localhost/milestones/abc'));
