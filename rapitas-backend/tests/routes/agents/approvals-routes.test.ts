...
mock.module('../../../services/core/github-service', () => ({
  GitHubService: class {
    constructor() {}
    createPR() {
      return Promise.resolve({ url: 'http://github.com/pr/1' });
    }
  },
}));
...
describe('Approvals Routes', () => {
  ...
  test('GET /approvals/:id - 承認詳細取得', async () => {
    const res = await app.handle(new Request('http://localhost/approvals/1'));
    expect(res.status).toBe(200);
    const body = await res.json();
  ...
  test('GET /approvals/:id - 存在しない場合', async () => {
    mockPrisma.approvalRequest.findUnique.mockImplementationOnce(() => Promise.resolve(null));
    const res = await app.handle(new Request('http://localhost/approvals/999'));
    expect(res.status).toBe(200);
