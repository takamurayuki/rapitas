...
describe('GET /tasks/:id/comments', () => {
  ...
  test('タスクのコメント一覧を返すこと', async () => {
    ...
    const res = await app.handle(new Request('http://localhost/tasks/1/comments'));
  ...
  test('無効なタスクIDで400を返すこと', async () => {
    const res = await app.handle(new Request('http://localhost/tasks/abc/comments'));
...
describe('DELETE /comments/:id', () => {
  ...
  test('コメントを削除すること', async () => {
    ...
    const res = await app.handle(new Request('http://localhost/comments/1', { method: 'DELETE' }));
    const body = await res.json();
