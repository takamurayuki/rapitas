...
describe('GET /system-prompts', () => {
  ...
  test('全システムプロンプトを返すこと', async () => {
    ...
    const res = await app.handle(new Request('http://localhost/system-prompts'));
  ...
  test('カテゴリでフィルタできること', async () => {
    const prompts = [{ id: 1, key: 'task_analysis', name: 'タスク分析', category: 'analysis' }];
    mockPrisma.systemPrompt.findMany.mockResolvedValue(prompts);

    const res = await app.handle(new Request('http://localhost/system-prompts?category=analysis'));
    const body = await res.json();
...
describe('GET /system-prompts/:key', () => {
  ...
  test('キーでシステムプロンプトを取得すること', async () => {
    ...
    const res = await app.handle(new Request('http://localhost/system-prompts/task_analysis'));
    const body = await res.json();
  ...
  test('存在しないキーで404を返すこと', async () => {
    ...
    const res = await app.handle(new Request('http://localhost/system-prompts/nonexistent'));
    const body = await res.json();
