...
const { UserBehaviorService } = await import(
  "../../src/services/userBehaviorService"
);
...
describe("createTask", () => {
  ...
  test("親タスク作成時にユーザー行動を記録すること", async () => {
    ...
    expect(UserBehaviorService.recordTaskCreated).toHaveBeenCalledWith(
      1,
      createdTask,
    );
  });
...
describe("updateTask", () => {
  ...
  test("存在しないタスクの更新でエラーすること", async () => {
    ...
    await expect(
      updateTask(mockPrisma as never, 999, { title: "Updated" }),
    ).rejects.toThrow("見つかりません");
  });
  ...
  test("完了時に行動記録と通知をトリガーすること", async () => {
    ...
    expect(UserBehaviorService.recordTaskCompleted).toHaveBeenCalledWith(
      1,
      updatedTask,
    );
  });
  ...
  test("存在しないタスクでエラーをスローすること", async () => {
    ...
    await expect(updateTask(mockPrisma as never, 999, { title: "Updated" }))
      .rejects
      .toThrow("タスク(ID: 999)が見つかりません");
...
describe("getFrequencyBasedSuggestions", () => {
  ...
  test("頻度順にソートして返すこと", async () => {
    const completedTasks = [
      { id: 1, title: "Daily Report", priority: "medium", estimatedHours: 1, completedAt: new Date("2026-03-01"), description: null, taskLabels: [] },
      { id: 2, title: "Daily Report", priority: "medium", estimatedHours: 1, completedAt: new Date("2026-03-02"), description: null, taskLabels: [] },
      { id: 3, title: "Weekly Review", priority: "high", estimatedHours: 2, completedAt: new Date("2026-03-03"), description: null, taskLabels: [] },
    ];
  ...
  test("既存タスクと重複する提案を除外すること", async () => {
    const completedTasks = [
      { id: 1, title: "Task A", priority: "medium", estimatedHours: 1, completedAt: new Date(), description: null, taskLabels: [] },
    ];
