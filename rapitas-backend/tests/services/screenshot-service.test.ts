...
describe("hasUIChanges", () => {
  ...
  test("Clientコンポーネントの変更でtrueを返すこと", () => {
    const result = hasUIChanges([
      "rapitas-frontend/src/app/tasks/TaskDetailClient.tsx",
    ]);
    expect(result).toBe(true);
  });
  ...
  test("ユーティリティファイルの変更でfalseを返すこと", () => {
    const result = hasUIChanges([
      "rapitas-frontend/src/utils/format-date.ts",
    ]);
  ...
  test("テストファイルの変更でfalseを返すこと", () => {
    const result = hasUIChanges([
      "rapitas-frontend/src/components/Button.test.tsx",
    ]);
    expect(result).toBe(false);
  });
  ...
  test("hooksファイルの変更でfalseを返すこと", () => {
    const result = hasUIChanges([
      "rapitas-frontend/src/hooks/use-dark-mode.ts",
    ]);
    expect(result).toBe(false);
  });
  ...
  test("型定義ファイルの変更でfalseを返すこと", () => {
    const result = hasUIChanges([
      "rapitas-frontend/src/types/task.d.ts",
    ]);
    expect(result).toBe(false);
  });
  ...
  test("共通コンポーネントの変更でfalseを返すこと", () => {
    const result = hasUIChanges([
      "rapitas-frontend/src/components/common/Button.tsx",
    ]);
    expect(result).toBe(false);
  });
});
...
describe("detectAffectedPages", () => {
  ...
  test("layout.tsxからルートパスを検出すること", () => {
    const pages = detectAffectedPages([
      "rapitas-frontend/src/app/settings/layout.tsx",
    ]);
    expect(pages).toHaveLength(1);
  ...
  test("動的ルート([id])をスキップすること", () => {
    const pages = detectAffectedPages([
      "rapitas-frontend/src/app/tasks/[id]/page.tsx",
    ]);
...
describe("detectPagesFromAgentOutput", () => {
  test("localhost URLからページパスを抽出すること", () => {
    const output = "I updated the page at http://localhost:3000/calendar and http://localhost:3000/settings";
