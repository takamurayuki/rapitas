...
describe('質問検出ロジック', () => {
        ...
        questions: [
          {
            ...
            header: 'Features',
            options: [{ label: '認証' }, { label: 'キャッシュ' }, { label: 'ログ' }],
            multiSelect: true,
          },
        ],
      };
    ...
    it('descriptionがないオプションを処理する', () => {
      const input = {
        questions: [
          {
            question: '選択してください',
            options: [{ label: 'オプション1' }, { label: 'オプション2' }],
          },
        ],
      };
      const result = extractQuestionInfo(input);
  ...
  describe('inferQuestionCategory', () => {
    ...
    it('確認系のキーワードが含まれる場合はconfirmationを返す', () => {
      const input = {
        questions: [{ question: '続けてもよろしいですか？' }],
      };
    ...
    it('デフォルトはclarificationを返す', () => {
      const input = {
        questions: [{ question: 'どのような機能が必要ですか？' }],
      };
...
describe('QuestionKeyの完全性チェック', () => {
  it('要件で指定されたフォーマットを満たす', () => {
    ...
    const key = createQuestionKeyFromToolCall({ questions: [{ question: 'テスト' }] }, 300);
