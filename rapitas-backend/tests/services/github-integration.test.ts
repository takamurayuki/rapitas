...
describe("GitHub Integration - Webhook Handling", () => {
  ...
  test("pull_request openedイベントを処理できること", async () => {
    ...
    await service.handleWebhook("pull_request", {
      action: "opened",
      repository: { name: "repo", html_url: "https://github.com/owner/repo", owner: { login: "owner" } },
      pull_request: {
        number: 1, title: "New PR", body: "description", state: "open",
        head: { ref: "feature" }, base: { ref: "main" },
        user: { login: "dev" }, html_url: "https://github.com/owner/repo/pull/1",
      },
    });
  ...
  test("issuesイベントを処理できること", async () => {
    await service.handleWebhook("issues", {
      ...
      repository: { name: "repo", html_url: "https://github.com/owner/repo", owner: { login: "owner" } },
      issue: {
        number: 5, title: "Bug", body: "fix this", state: "open",
  ...
  test("PR同期が正常にカウントを返すこと", async () => {
    mockPrisma.gitHubIntegration.findUnique.mockResolvedValue({
      id: 1, ownerName: "owner", repositoryName: "repo",
    });
    mockExecAsync.mockResolvedValue({
      stdout: JSON.stringify([{
        number: 10, title: "Sync PR", body: null, state: "OPEN",
        headRefName: "dev", baseRefName: "main",
        author: { login: "dev" }, url: "https://github.com/owner/repo/pull/10",
