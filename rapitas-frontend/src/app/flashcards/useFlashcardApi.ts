...
export function useFlashcardApi(
  ...
  const fetchDeck = useCallback(async (id: number) => {
    try {
  ...
  const handleDeleteDeck = useCallback(async (id: number) => {
  ...
  }, [selectedDeck, currentCardIndex, fetchDeck, setSchedulePreview, setCurrentCardIndex, setIsStudyMode]);

  /** Fetch the spaced-repetition schedule preview for a card. */
  const fetchSchedulePreview = useCallback(async (cardId: number) => {
    try {
  ...
  const handleGenerateCards = useCallback(async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    generateTopic, generateCount, generateDifficulty, generateLanguage,
    selectedDeck, fetchDeck, setIsGenerateModalOpen, setGenerateTopic,
    setGenerateCount, setGenerateDifficulty, setIsGenerating, t,
  ]);
