...
import { useAutoWindowHeight } from '../_hooks/use-auto-window-height';
...
export default function PomodoroFloatView() {
  ...
  // Show the full timer panel (ring, tomatoes, start button) whenever a task
  // is known — running OR handed over from the task detail page — so the idle
  // window already looks like a Pomodoro timer (operator request 2026-09-02).
  // The empty state only remains for a float with no task at all.
  const displayTaskId = state.isTimerRunning ? state.taskId : state.lastUsedTaskId;
  ...
  return (
    <div
      className={`fixed inset-0 overflow-y-auto rounded-xl [scrollbar-width:none] ${surfaceCls} ${textShadowCls}`}
    >
      {/* Natural-height wrapper: its scrollHeight drives the window height
          (useAutoWindowHeight), so the fixed root never actually scrolls. */}
      <div ref={contentRef} className="flex flex-col">
        ...
        {!showTimer ? (
          ...
          <PomodoroPanelContent
            ...
            taskTitle={displayTaskTitle ?? t('taskDefaultName')}
            focusMode={focusMode}
          />
        )}
      </div>
