...
export default function ClaudeMdGeneratorPage() {
  ...
  // Update local states when answers change
  useEffect(() => {
    ...
    return (
      ...
      >
          ...
          >
            {SCALES.map((s) => {
              ...
              return (
                <div
                  key={s.id}
                  className={`card ${sel2 ? 'sel' : ''}`}
                  onClick={() => setLocalScale(s.id)}
                >
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          <button className="btn btn-g" onClick={() => setPhase('elements')}>
            {t('back')}
          </button>
          <button
            className="btn btn-p"
