...
import { useHomeSyncEffects } from './useHomeSyncEffects';
...
function HomeClientPage() {
  ...
  const fetchTasks = useCallback(async () => {
    if (taskCacheInitialized) { await fetchTaskUpdates(); }
    else { await fetchAllTasks(); }
  }, [taskCacheInitialized, fetchTaskUpdates, fetchAllTasks]);

  useTaskAutoSync({ enabled: true, interval: 30000, silent: true, skipDuringExecution: true });

  useHomeInit({
    taskCacheInitialized, fetchAllTasks, fetchTaskUpdates,
    initializeFilterData, categoryFilter, categories,
    setCategoryFilter, setGlobalSettings,
  });

  const { filteredTasks, statusCounts, todayTasksCounts } = useFilteredTasks({
    tasks, filter, categoryFilter, themeFilter, priorityFilter,
    searchQuery: debouncedSearchQuery, themes,
  });
  const sortedTasks = useTaskSorting({ tasks: filteredTasks, sortBy, sortOrder });
  const completedTasksCount = todayTasksCounts.completed;
  const totalTasksCount = todayTasksCounts.total;

  const {
    themeScrollRef, canScrollLeft, canScrollRight, isScrollNeeded,
    scrollThemeLeft, scrollThemeRight,
  ...
  const { updateStatus, handleQuickAdd, toggleTaskSelection, bulkUpdateStatus, bulkDelete } =
    useHomeActions({
      tasks, themes, categoryFilter, themeFilter, defaultTheme,
      isSelectionMode, selectedTasks, setSelectedTasks, setIsSelectionMode,
      setIsQuickAdding, setQuickTaskTitle,
      triggerTaskCompletion, isTodayTask, fetchTasks,
    });
  ...
  useHomeSyncEffects({
    ...
    themes, visibleCategories, setCategoryFilter, setThemeFilter, setDefaultTheme,
  ...
  useExecutingTasksPolling({
    ...
    onExecutingTaskFound: useCallback(
      (taskId: number) => { if (!isPanelOpen) openTaskPanel(taskId); },
  ...
  const handleSelectAll = () => {
    ...
    if (allSelected) { setSelectedTasks(new Set()); setIsSelectionMode(false); }
    ...
    <div className="h-[calc(100vh-4.2rem)] overflow-auto bg-background">
      <div className="mx-auto max-w-6xl px-4 py-4">
        ...
        <HomeQuickAdd
          ...
          onCancel={() => { setIsQuickAdding(false); setQuickTaskTitle(''); }}
        />
