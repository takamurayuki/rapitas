      ...
      <div className="bg-gradient-to-r from-orange-500 to-amber-500 rounded-2xl p-5 text-white">
        <div className="flex flex-wrap items-center gap-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-3xl font-bold">{studyStreak.currentStreak}</div>
          ...
          <div className="flex gap-6 text-sm">
            <div>
              <div className="font-semibold text-lg">{studyStreak.todayMinutes}min</div>
              <div className="opacity-80">today</div>
            </div>
            <div>
              <div className="font-semibold text-lg">{studyStreak.todayTasksCompleted}</div>
              <div className="opacity-80">tasks done</div>
            </div>
            <div>
              <div className="font-semibold text-lg">{studyStreak.weeklyMinutes}min</div>
              <div className="opacity-80">this week</div>
            </div>
            <div>
              <div className="font-semibold text-lg">{studyStreak.longestStreak}</div>
      ...
      {examGoals.length > 0 && (
        <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {examGoals.filter((g) => !g.isCompleted).map((goal) => (
              <div
                key={goal.id}
                className="bg-white dark:bg-zinc-800 rounded-xl p-4 border border-zinc-200 dark:border-zinc-700"
              >
                <div className="flex items-center justify-between mb-2">
                  <h3 className="font-medium text-zinc-900 dark:text-zinc-100">{goal.name}</h3>
                  <span
                ...
                <div className="flex items-center gap-3 text-sm text-zinc-500 dark:text-zinc-400 mb-3">
                  {goal.targetScore && <span>Target: {goal.targetScore}</span>}
      ...
      {learningGoals.length > 0 && (
        <section>
          ...
          <div className="space-y-3">
            {learningGoals.filter((g) => g.status === 'active').map((goal) => (
              <div
                key={goal.id}
                className="bg-white dark:bg-zinc-800 rounded-xl p-4 border border-zinc-200 dark:border-zinc-700"
              >
                <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
                  <h3 className="font-medium text-zinc-900 dark:text-zinc-100">{goal.title}</h3>
