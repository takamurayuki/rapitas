...
export default function NewTaskClient() {
  ...
  const lastAutoGeneratedDescRef = useRef<string>("");
  const isGeneratingRef = useRef(false);

  const autoGenerateTitleFromDesc = useCallback(async (desc: string) => {
    if (!desc.trim() || isGeneratingRef.current) return;
    if (desc.trim() === lastAutoGeneratedDescRef.current) return;
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.title) {
          setTitle(data.title);
          lastAutoGeneratedDescRef.current = desc.trim();
        }
      }
    } catch (e) {
      console.error("Auto generate title failed:", e);
    } finally {
  ...
  useEffect(() => {
    if (!globalSettings?.autoGenerateTitle) return;
    if (!description.trim()) return;
    // タイトルが手動入力済みの場合はスキップ
    if (title.trim() && lastAutoGeneratedDescRef.current === "") return;
    ...
    autoGenerateTimerRef.current = setTimeout(() => {
      autoGenerateTitleFromDesc(description);
    }, 1500);
  ...
  }, [description, globalSettings?.autoGenerateTitle, autoGenerateTitleFromDesc]);
          ...
          <CompactAccordionGroup
            ...
            headerExtra={
              globalSettings?.autoGenerateTitle ? (
                <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium bg-violet-50 dark:bg-violet-900/30 text-violet-500 dark:text-violet-400">
                  {isGeneratingTitle ? (
                    <Loader2 className="w-3.5 h-3.5 animate-spin" />
                  ) : (
                    <Sparkles className="w-3.5 h-3.5" />
                  )}
                  {isGeneratingTitle ? "タイトル生成中..." : "タイトル自動生成"}
                </div>
              ) : (
                <button
                  type="button"
