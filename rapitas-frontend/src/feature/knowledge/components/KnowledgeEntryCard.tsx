...
export function KnowledgeEntryCard({ entry, similarity, onClick, onArchive }: KnowledgeEntryCardProps) {
    ...
    >
      <div className="flex items-start justify-between gap-2">
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          <span className={`text-xs px-1.5 py-0.5 rounded-full ${stageColors[entry.forgettingStage] ?? ""}`}>
            {t(`stages.${entry.forgettingStage}`)}
          </span>
          <span className={`text-xs px-1.5 py-0.5 rounded-full ${validationColors[entry.validationStatus] ?? ""}`}>
