...
export type UseGitHubIntegrationReturn = {
  ...
  createTaskFromIssue: (
    issueId: number,
    options?: { projectId?: number; themeId?: number },
  ) => Promise<{ taskId: number; task: unknown }>;
  createIssueFromTask: (
    taskId: number,
    integrationId: number,
    labels?: string[],
...
export function useGitHubIntegration(): UseGitHubIntegrationReturn {
  ...
  const getPullRequestDiff = useCallback(
    (id: number) => {
  ...
  const approvePullRequest = useCallback(
  ...
  const linkPullRequestToTask = useCallback(
    async (taskId: number, prId: number) => {
