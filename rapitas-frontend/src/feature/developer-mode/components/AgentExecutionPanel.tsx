...
export function AgentExecutionPanel({
  ...
  const detectQuestion = (logText: string): { hasQuestion: boolean; question: string } => {
    ...
    const hasQuestion = questionPatterns.some(pattern => pattern.test(lastLines));

    if (hasQuestion) {
      // 質問部分を抽出
      const questionLines = lines.slice(-3).filter(l =>
        questionPatterns.some(p => p.test(l)) || l.trim().endsWith("?")
      );
      return { hasQuestion: true, question: questionLines.join("\n") || lastLines };
    }
  ...
  const isWaitingForInput = pollingStatus === "waiting_for_input" || pollingWaitingForInput || hasQuestion;

  // 実行開始時にポーリングを開始
  useEffect(() => {
    console.log("[AgentExecutionPanel] executionResult changed:", executionResult);
    console.log("[AgentExecutionPanel] isExecuting:", isExecuting);
    if (executionResult?.success && executionResult?.sessionId) {
      console.log("[AgentExecutionPanel] Starting polling for session:", executionResult.sessionId);
      startPolling();
    }
  ...
  // 実行中または完了後（ログあり）
  const showLogPanel = (isExecuting || isPollingRunning || logs.length > 0) &&
    (executionStatus === "completed" || isExecuting || pollingStatus === "running" || isWaitingForInput);

  // 実行完了後のステータス判定
  const finalStatus = pollingStatus !== "idle" ? pollingStatus : executionStatus;
  // waiting_for_inputの場合は完了とは見なさない
  const isCompleted = finalStatus === "completed" && !isPollingRunning && !isWaitingForInput;
  const isFailed = finalStatus === "failed" || error || pollingError;
  // waiting_for_inputの場合も実行中として扱う（応答入力を待っている）
  const isRunning = isExecuting || isPollingRunning || pollingStatus === "running" || isWaitingForInput;
    ...
    >
      <div className="flex items-center justify-between px-4 py-2 bg-zinc-800 rounded-t-lg border-b border-zinc-700">
        ...
        <div className="flex items-center gap-1">
          ...
          >
            {isLogExpanded ? <Minimize2 className="w-4 h-4" /> : <Maximize2 className="w-4 h-4" />}
          </button>
        </div>
      </div>
      <div
      ...
      >
        <pre className="p-4 text-zinc-300 whitespace-pre-wrap break-words">
          {logs.length > 0 ? (
            logs.map((log, i) => (
              <span key={i} className={log.includes("[エラー]") ? "text-red-400" : ""}>
                {log}
              </span>
            ))
          ) : (
  ...
  if (isCompleted && executionResult?.success) {
    return (
      <div className="bg-gradient-to-br from-emerald-50 to-green-50 dark:from-emerald-950/30 dark:to-green-950/30 rounded-xl border border-emerald-200 dark:border-emerald-800 overflow-hidden">
        <div className="p-6">
          <div className="flex items-start gap-4">
  ...
  if (isFailed) {
    return (
      <div className="bg-gradient-to-br from-red-50 to-rose-50 dark:from-red-950/30 dark:to-rose-950/30 rounded-xl border border-red-200 dark:border-red-800 overflow-hidden">
        <div className="p-6">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-red-600 dark:text-red-400 mt-1">
                {error || pollingError || executionResult?.error || "不明なエラーが発生しました"}
              </p>
            </div>
            <button
              onClick={handleReset}
  ...
  // 初期状態（実行ボタン）
  return (
    <div className="bg-gradient-to-br from-violet-50 via-purple-50 to-indigo-50 dark:from-violet-950/30 dark:via-purple-950/30 dark:to-indigo-950/30 rounded-xl border border-violet-200 dark:border-violet-800 overflow-hidden">
      {/* メインセクション */}
      <div className="p-6">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-zinc-600 dark:text-zinc-400 mt-1">
              Claude Codeがこのタスクを自動で実装します。完了後、差分をレビューしてコミット・PRを作成できます。
            </p>
          </div>
          <button
