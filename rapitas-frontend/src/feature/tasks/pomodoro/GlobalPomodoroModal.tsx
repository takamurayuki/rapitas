...
import { Timer, ExternalLink, X, Settings, Volume2, VolumeX, BarChart3, ChevronDown, ChevronUp } from "lucide-react";
          ...
          {showSettings && (
            <div className="px-4 pb-4 space-y-4">
                ...
                {state.settings.soundEnabled && (
                  <div className="flex items-center gap-3">
                    ...
                    <input
                      ...
                      onChange={(e) => state.updateSettings({ soundVolume: parseFloat(e.target.value) })}
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className="block text-xs text-zinc-500 dark:text-zinc-400 mb-1">作業時間</label>
                  <select
                    value={state.settings.pomodoroDuration / 60}
                    onChange={(e) => state.updateSettings({ pomodoroDuration: parseInt(e.target.value) * 60 })}
                ...
                <div>
                  <label className="block text-xs text-zinc-500 dark:text-zinc-400 mb-1">短い休憩</label>
                  <select
                    value={state.settings.shortBreakDuration / 60}
                    onChange={(e) => state.updateSettings({ shortBreakDuration: parseInt(e.target.value) * 60 })}
                    className="w-full px-2 py-1.5 text-sm bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg"
                  >
                    {[3, 5, 10, 15].map((min) => (
                      <option key={min} value={min}>{min}分</option>
                ...
                <div>
                  <label className="block text-xs text-zinc-500 dark:text-zinc-400 mb-1">長い休憩</label>
                  <select
                    value={state.settings.longBreakDuration / 60}
                    onChange={(e) => state.updateSettings({ longBreakDuration: parseInt(e.target.value) * 60 })}
                    className="w-full px-2 py-1.5 text-sm bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg"
                  >
                    {[10, 15, 20, 30].map((min) => (
                      <option key={min} value={min}>{min}分</option>
