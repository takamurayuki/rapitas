...
export default function TaskTimeTracking({
  ...
  const remainingHours =
    estimatedHours && actualHours ? Math.max(0, estimatedHours - actualHours) : estimatedHours || 0;

  const progressPercent =
    estimatedHours && actualHours ? Math.min((actualHours / estimatedHours) * 100, 100) : 0;
        ...
        <div className="w-full bg-zinc-200 dark:bg-zinc-700 rounded-full h-2 mb-4">
          <div
            className={`h-2 rounded-full transition-all ${
              isBreakTime ? 'bg-green-500' : 'bg-indigo-500'
            }`}
            style={{
              width: `${
                (((isBreakTime
        ...
        <div className="flex gap-2">
          {isTimerRunning ? (
            ...
            <button
              ...
              className="w-full px-4 py-2 bg-indigo-500 text-white rounded-lg hover:bg-indigo-600 font-medium transition-colors flex items-center justify-center gap-2"
            >
              <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                <path
                  fillRule="evenodd"
