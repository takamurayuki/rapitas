...
const TaskTitleAutocomplete = forwardRef<TaskTitleAutocompleteRef, TaskTitleAutocompleteProps>(
  function TaskTitleAutocomplete(
    ...
    const inlineSuggestion = useMemo(() => {
      ...
      const match = suggestions.find((s) => s.title.toLowerCase().startsWith(lowerValue));
    ...
    const fetchSuggestions = useCallback(
      async (query: string) => {
        ...
        try {
          ...
          logger.debug('Autocomplete fetch failed (backend might not be running):', e);
          setSuggestions([]);
        }
      },
      [themeId, projectId],
    ...
    const handleKeyDown = (e: KeyboardEvent<HTMLInputElement>) => {
      ...
      if (e.key === 'ArrowRight' && ghostText && inlineSuggestion && inputRef.current) {
    ...
    return (
      <div className="relative w-full">
        ...
        <div className="absolute inset-0 pointer-events-none flex items-center" aria-hidden="true">
          <span className="text-xl font-semibold text-transparent whitespace-pre">{value}</span>
          {isFocused && ghostText && (
        ...
        {/* Tab hint */}
        {isFocused && ghostText && (
          <div className="absolute right-0 top-1/2 -translate-y-1/2 flex items-center gap-1.5 text-xs text-zinc-500 dark:text-zinc-500 pointer-events-none">
