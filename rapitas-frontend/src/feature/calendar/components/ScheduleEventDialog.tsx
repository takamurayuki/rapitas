...
export default function ScheduleEventDialog({ selectedDate, onClose, onSubmit }: Props) {
  ...
  return (
    ...
    >
      ...
      >
        {/* Color accent bar */}
        <div className="h-1.5 sm:rounded-t-2xl rounded-t-2xl transition-colors duration-200" style={{ backgroundColor: color }} />
            ...
            <div className="flex items-center gap-3">
              ...
              >
                <span className="text-[10px] leading-none opacity-80 uppercase">
                  {new Date(startDate).toLocaleDateString(dateLocale, { month: 'short' })}
                </span>
                <span className="text-lg leading-none font-bold">{new Date(startDate).getDate()}</span>
              </div>
              <div>
        ...
        <form onSubmit={handleSubmit} className="px-5 pt-4 pb-5">
          ...
          <div className="mt-4 space-y-3">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <button type="button" onClick={() => setIsAllDay(true)}
              ...
              >終日</button>
              <button type="button" onClick={() => setIsAllDay(false)}
              ...
              >
                <Clock className="w-3.5 h-3.5" />時間指定
              </button>
              <div className="h-4 w-px bg-zinc-200 dark:bg-zinc-600" />
              <button type="button"
                onClick={() => { setIsMultiDay(!isMultiDay); if (!isMultiDay) setEndDate(startDate); }}
              ...
              >
                <CalendarDays className="w-3.5 h-3.5" />複数日
              </button>
            </div>
                ...
                <div className="flex gap-1.5">
                  {QUICK_TIMES.map((qt) => (
                    <button key={qt.label} type="button"
                      onClick={() => { setStartTime(qt.start); setEndTime(qt.end); }}
