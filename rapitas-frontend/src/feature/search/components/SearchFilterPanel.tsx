...
export default function SearchFilterPanel({ filters, onFilterChange }: SearchFilterPanelProps) {
  return (
    <div className="flex flex-col gap-4 p-3 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg">
      <div>
        <h4 className="text-xs font-semibold text-zinc-500 dark:text-zinc-400 uppercase mb-2">種類</h4>
          ...
          {TYPE_OPTIONS.map((opt) => (
            <label key={opt.value} className="flex items-center gap-1.5 text-sm text-zinc-700 dark:text-zinc-300 cursor-pointer">
              <input
                ...
                onChange={() => onFilterChange({ ...filters, types: toggleValue(filters.types, opt.value) })}
      ...
      <div>
        <h4 className="text-xs font-semibold text-zinc-500 dark:text-zinc-400 uppercase mb-2">ステータス</h4>
        <div className="flex flex-wrap gap-2">
          {STATUS_OPTIONS.map((opt) => (
            <label key={opt.value} className="flex items-center gap-1.5 text-sm text-zinc-700 dark:text-zinc-300 cursor-pointer">
              <input
                ...
                onChange={() => onFilterChange({ ...filters, statuses: toggleValue(filters.statuses, opt.value) })}
                className="rounded border-zinc-300 dark:border-zinc-600 text-indigo-600 focus:ring-indigo-500"
              />
              {opt.label}
            </label>
