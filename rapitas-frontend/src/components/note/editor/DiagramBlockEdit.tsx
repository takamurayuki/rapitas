  ...
  return (
    <div className="absolute inset-0 z-50 bg-white dark:bg-zinc-900 flex flex-col">
      ...
      <div className="flex items-center gap-1.5 px-4 py-2 border-b border-zinc-100 dark:border-zinc-800 overflow-x-auto shrink-0">
        <span className="text-[11px] text-zinc-400 dark:text-zinc-500 shrink-0">テンプレート:</span>
        {TEMPLATES.map((t) => (
          <button
            key={t.label}
            onClick={() => handleChange(t.source)}
        ...
        <div className="w-1/2 flex flex-col">
          ...
          <div className="flex-1 overflow-auto p-4 flex items-start justify-center">
            {previewError ? (
              ...
              <div className="text-sm text-zinc-400 dark:text-zinc-500">レンダリング中...</div>
            )}
          </div>
        </div>
      </div>
