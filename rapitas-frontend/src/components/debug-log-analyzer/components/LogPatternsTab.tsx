  ...
  return (
    <TabsContent value="patterns" className="space-y-4">
      ...
      {patterns.errors.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle>{t('debugLogAnalyzer.patterns.frequentErrors')}</CardTitle>
          </CardHeader>
          <CardContent>
              ...
              {patterns.errors.slice(0, 5).map((pattern, index) => (
                <div key={index} className="border-l-4 border-red-500 pl-4">
                  <div className="flex justify-between items-start">
                    ...
                    >
                      {t('debugLogAnalyzer.patterns.occurrenceCount', { count: pattern.count })}
                    </Badge>
                  </div>
                </div>
              ))}
      ...
      {patterns.warnings.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle>{t('debugLogAnalyzer.patterns.frequentWarnings')}</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
      ...
      <Card>
        <CardHeader>
          <CardTitle>{t('debugLogAnalyzer.patterns.frequentMessages')}</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-3">
